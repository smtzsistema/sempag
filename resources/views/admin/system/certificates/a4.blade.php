{{-- resources/views/admin/system/certificates/a4.blade.php --}}
@extends('admin.layouts.app')

@php
    $isEdit = !empty($certificate);
    $pageW = (int) (($initialConfig['page']['w'] ?? 1123));
    $pageH = (int) (($initialConfig['page']['h'] ?? 794));
@endphp

@section('title', $isEdit ? 'Editar certificado' : 'Novo certificado')
@section('breadcrumb', 'Admin • Configurações de Sistema')
@section('page_title', $isEdit ? 'Editar certificado' : 'Novo certificado')

@section('top_actions')
    <a href="{{ route('admin.system.certificates.index', $event) }}"
       class="rounded-xl bg-zinc-900 hover:bg-zinc-800 border border-zinc-800 px-4 py-2 text-sm transition">
        Voltar
    </a>

    @if($isEdit)
        <form method="POST" action="{{ route('admin.system.certificates.destroy', [$event, $certificate]) }}"
              class="inline">
            @csrf
            <button type="submit"
                    onclick="return confirm('Excluir este certificado?')"
...
@section('content')
    ...
    @php
        $action = $isEdit
            ? route('admin.system.certificates.update', [$event, $certificate])
            : route('admin.system.certificates.storeA4', $event);

        $existingBg = $isEdit && !empty($certificate->cer_fundo)
            ? asset('storage/' . $certificate->cer_fundo)
            : null;

        $selected = old('category_ids', $selectedCategories ?? []);
        $mirrorOld = old('cer_espelhar', ($isEdit && ($certificate->cer_espelhar ?? 'N') === 'S') ? 1 : 0);
    @endphp
                    ...
                    <div class="p-5 border-b border-zinc-800 flex items-center justify-between gap-3">
                        <div>
                            ...
                            <div class="text-xs text-zinc-400 mt-0.5">
                                ({{ $pageW }}×{{ $pageH }}). Arrasta os itens com o mouse.
                            </div>
                        </div>
            ...
            <div class="lg:col-span-5">
                <div class="rounded-2xl bg-zinc-900 border border-zinc-800 overflow-hidden">
                    <div class="p-5 border-b border-zinc-800">
                        ...
                        <div class="text-xs text-zinc-400 mt-0.5">Tudo que você mexer aqui vai pro JSON do certificado.
                        </div>
                    </div>
                    ...
                    <div class="p-5 space-y-4">
                        ...
                        <div>
                            ...
                            <input type="text"
                                   name="cer_nome"
                                   value="{{ old('cer_nome', $isEdit ? $certificate->cer_nome : '') }}"
                                   ...
                                   placeholder="Ex.: Certificado Participante">
                            @error('cer_nome')
                        ...
                        {{-- bg upload --}}
                        <div>
                            <label class="text-sm text-zinc-300">Imagem de fundo</label>
                            <input type="file"
                                   name="cer_fundo"
                        ...
                        {{-- hidden config --}}
                        <input type="hidden" name="cer_config" id="cb-config"
                               value="{{ old('cer_config', json_encode($initialConfig ?? [], JSON_UNESCAPED_UNICODE)) }}">
    ...
    <script>
        (function () {
            ...
            const PAGE_W = (window.__CB_INITIAL?.page?.w ?? 1123) | 0;
            const PAGE_H = (window.__CB_INITIAL?.page?.h ?? 794) | 0;
