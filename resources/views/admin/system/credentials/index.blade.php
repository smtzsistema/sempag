...
@section('page_title', 'Credenciais')

@section('top_actions')
    <a href="{{ route('admin.system.credentials.create', $event) }}"
    ...
    <div class="rounded-2xl bg-zinc-900 border border-zinc-800 overflow-hidden">
        <div class="p-5 border-b border-zinc-800">
            <div class="text-sm text-zinc-400">
                Aqui você configura modelos de credencial e vincula por categoria(s). Por enquanto, só A4.
            </div>
        </div>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full text-sm">
                ...
                @forelse($credentials as $cred)
                    @php
                        $catNames = collect($cred->category_names ?? []);
                        $tipo = strtoupper($cred->cre_tipo ?? 'A4');
                        $espelhar = ($cred->cre_espelhar ?? 'N') === 'S';
                    @endphp
                    <tr class="hover:bg-zinc-950/30">
                        <td class="px-4 py-3">
                            <div class="font-semibold text-zinc-100">{{ $cred->cre_nome }}</div>
                            @if(!empty($cred->cre_fundo))
                        ...
                        <td class="px-4 py-3 text-xs text-zinc-400">
                            {{ optional($cred->updated_at)->format('d/m/Y H:i') }}
                        </td>

                        <td class="px-4 py-3 text-right">
                            <a href="{{ route('admin.system.credentials.edit', [$event, $cred]) }}"
