...
<head>
    ...
    <title>Escolher certificado - {{ $event->name }}</title>
...
<div class="max-w-4xl mx-auto p-6">
    <div class="rounded-2xl bg-zinc-900 border border-zinc-800 p-6">
        <div class="flex items-start justify-between gap-4">
            <div>
                <h1 class="text-2xl font-bold">Escolher certificado</h1>
        ...
        <div class="mt-6 grid md:grid-cols-2 gap-4">
            @foreach($certificates as $cert)
                <a href="{{ route('public.attendee.certificate.print', [$event, $cert->cer_id]) }}"
                   class="block rounded-2xl bg-zinc-950 border border-zinc-800 p-5 hover:border-zinc-700 transition">
                    <div class="text-lg font-semibold">{{ $cert->cer_nome }}</div>
